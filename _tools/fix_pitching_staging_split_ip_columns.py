...
def get_html_year_row(pid, year, old_row):
    key = (pid, str(year))
    ...
    for table_idx, table in enumerate(soup.find_all("table")):
        ...
        for tr_idx, tr in enumerate(table.find_all("tr")):
            ...
            for col, idx in {"G": 2, "W": 3, "L": 4, "SV": 5}.items():
                if norm_num(old_row.get(col, "")) and idx < len(cells) and norm_num(old_row.get(col, "")) == norm_num(cells[idx]):
                    score += 10

            candidates.append((score, table_idx, tr_idx, cells))
    ...
    _html_cache[key] = candidates[0][3]
    return candidates[0][3]

def is_split_ip_row(cells):
    if not cells or len(cells) < 26:
        return False

    return (
        re.match(r"^\d+\s+\.[12]$", norm(cells[13])) is not None
        and re.match(r"^\d+$", norm(cells[14])) is not None
        and re.match(r"^\.[12]$", norm(cells[15])) is not None
    )

def fixed_from_split_ip(cells):
    # split-IP行では [14], [15] が投球回セル内部の重複なので、
    # 以後の実データは [16] から始まる
    return {
...
def main():
    ...
    report_path = report_dir / f"fix_split_ip_columns_report_{run_id}.csv"

    backup = None
    if args.apply:
        backup = root / f"_data/master_csv__rescrape_staging__backup_splitip_{run_id}"
    ...
    for path in sorted(staging.glob("pitching_*_from_master.csv")):
        ...
        for row in rows:
            ...
            cells = get_html_year_row(pid, year, row)
            ...
            if not is_split_ip_row(cells):
                counters["non_split_ip_row"] += 1
                new_rows.append(row)
                continue

            counters["split_ip_row"] += 1

            fixed = fixed_from_split_ip(cells)
            new_row = dict(row)
            changed = []
    ...
    if not args.apply:
        ...
        print("python _tools/fix_pitching_staging_split_ip_columns.py --apply")
