...
out_path = out_dir / "era_value_matches_html_column_sample_v2.csv"
...
for s in picked:
    ...
    if not row:
        counter["CSV_ROW_NOT_FOUND"] += 1
        continue

    csv_era_raw = row.get("ERA") or row.get("era") or row.get("防御率") or ""
    csv_era = norm_num(csv_era_raw)

    if not csv_era:
        counter["CSV_ERA_EMPTY"] += 1
        continue
        ...
        for i, cell in enumerate(cells):
            if norm_num(cell) == csv_era:
                ...
                rows_out.append({
                    ...
                    "csv_ERA_raw": csv_era_raw,
                    "csv_ERA_norm": csv_era,
    ...
    if not matched:
        ...
        rows_out.append({
            ...
            "csv_ERA_norm": csv_era,
            "matched_html_index": "",
            "matched_html_header": "NO_MATCH",
            "matched_html_value": "",
            "table_idx": "",
            "html_header": "",
            "html_row": " / ".join(["|".join(h[2]) for h in hits[:3]]),
            "csv_file": str(csv_path),
            "line": line_no,
        })
...
with out_path.open("w", encoding="utf-8-sig", newline="") as f:
    w = csv.DictWriter(f, fieldnames=[
        ...
        "csv_ERA_raw", "csv_ERA_norm",
        "matched_html_index", "matched_html_header", "matched_html_value",
        "table_idx", "html_header", "html_row", "csv_file", "line"
    ])
    w.writeheader()
    w.writerows(rows_out)

print()
print("=== CSV ERA value matched HTML column sample v2 ===")
for k, v in counter.most_common(40):
    print(v, k)

print()
print("matched/detail rows:", len(rows_out))
