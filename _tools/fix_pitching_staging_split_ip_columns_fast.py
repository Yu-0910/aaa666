...
def is_pitching_table(table):
    ...
    return all(x in text for x in ["投球回", "自責点", "防御率"])
...
def parse_player_html(pid):
    ...
    for table_idx, table in enumerate(soup.find_all("table")):
        if not is_pitching_table(table):
            continue
...
def get_year_cells(pid, year, old_row):
    if pid not in player_cache:
        player_cache[pid] = parse_player_html(pid)

    candidates = player_cache[pid].get(str(year), [])
    ...
    for cells in candidates:
        ...
        for col, idx in {"G": 2, "W": 3, "L": 4, "SV": 5}.items():
            if idx < len(cells) and norm_num(old_row.get(col, "")) and norm_num(old_row.get(col, "")) == norm_num(cells[idx]):
                score += 10
...
def main():
    ...
    for path in sorted(staging.glob("pitching_*_from_master.csv")):
        ...
        for row in rows:
            ...
            cells = get_year_cells(pid, year, row)
    ...
    print()
    print("=== Split-IP column fix FAST ===")
    ...
    if not args.apply:
        ...
        print("python _tools/fix_pitching_staging_split_ip_columns_fast.py --apply")
