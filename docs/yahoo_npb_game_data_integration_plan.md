...
| 6 | **ソース間の優先（矛盾時）** | **詳細事実**: 一球速報 > 出場成績の明細 > テキスト速報。**イニング得点など全体**: スコアボード／出場成績サマリを優先し、テキストは補助。矛盾は **ログ＋フラグ**（黙って上書きしない）。 | Phase 2 の正規化、Phase 3 の検証ロジックの実装方針が決まる。 |
...
| 盗塁・エラー・犠打など **テキスト速報の曖昧さ** | テキストは補助とし、可能なら一球データと **突合** 。矛盾時はフラグ付きでログ。 |
...
|--------------------------|------------|
| **打席別成績（1〜5巡目以上）** | `getPaOrderRow` が **実データを読まず**、条件に合う行だけ **固定値 `.000` のダミー行**を返している（`app/components/SeasonStatsPilot.tsx`）。**巡目別の正しい集計ではない**。 |
