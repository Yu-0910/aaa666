...
export function isSportsnaviMainGameCancelled(html, gameId = "") {
  ...
  // 試合カード状態
  if (/class="bb-gameCard__state"[\s\S]{0,160}(試合中止|ノーゲーム)/i.test(html)) return true

  const gid = String(gameId || "").trim()
  if (gid) {
    // 当該試合の scoreList 状態（リンクなし）
    if (new RegExp(`class="bb-scoreList__state">\\s*(試合中止|ノーゲーム)\\s*<`, "i").test(html)) {
      return true
    }
