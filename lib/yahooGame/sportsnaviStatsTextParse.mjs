...
/**
 * 試合トップ HTML で当該カードが「試合が成立しない/成立しなかった」ことが明示されているか。
 *
 * 対象:
 * - 試合中止
 * - ノーゲーム
 *
 * 注意:
 * - 別試合への `bb-scoreList__state`（一覧の状態表示）は含めない。
 * - `bb-head01__title`（当該試合ページ固有の見出し）のみを見る。
 *
 * @param {string} html
 */
export function isSportsnaviMainGameCancelled(html) {
  if (!html || typeof html !== "string") return false
  return /<h2[^>]*\bbb-head01__title\b[^>]*>\s*(試合中止|ノーゲーム)\s*<\/h2>/i.test(html)
}
 ...
 */
export function buildBattingPitchingFromStatsRows(rows) {
  const battingLines = []
  const pitchingLines = []
  for (const r of rows) {
