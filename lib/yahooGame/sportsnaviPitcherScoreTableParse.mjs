...
export function parsePitcherScoreTableRowFromTrInner(trInner) {
  ...
  const hasStateCell = !/\/npb\/player\/\d+\//.test(tdInners[0] ?? "")
  const state = hasStateCell ? stripTags(tdInners[0]).replace(/\s+/g, " ").trim() : ""
  const labels = tdInners.slice(hasStateCell ? 2 : 1).map(scoreLabelFromTd)
  const era = labels[0] ?? ""
  const ip = labels[1] ?? ""
