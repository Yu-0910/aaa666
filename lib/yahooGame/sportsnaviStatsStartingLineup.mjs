...
const PLAYER_NAME_TO_YAHOO_ID_FALLBACK = new Map([
  // Sportsnavi occasionally renders this row as plain <span> text instead of an
  // /npb/player/{id}/ link (for example 2021038804).
  ["ビシエド", "1600021"],
])
...
function fallbackYahooPlayerIdForName(playerName) {
  const key = normalizePlayerNameKey(playerName)
  return PLAYER_NAME_TO_YAHOO_ID_FALLBACK.get(key) ?? null
}
 ...
 * @returns {{ yahooPlayerId: string | null, playerName: string, cells: string[], positionCell: string } | null}
 */
export function parseStatsPlayerRowFromTrInner(trInner) {
  ...
  for (let i = 0; i < tdParts.length; i++) {
    if (/npb\/player\/\d+\//.test(tdParts[i].inner) || /\bbb-statsTable__data--player\b/.test(tdParts[i].attrs)) {
      playerIdx = i
      break
    }
  }
  ...
  const playerCell = tdParts[playerIdx].inner
  const idMatch = playerCell.match(/\/npb\/player\/(\d+)\//) ?? trInner.match(/\/npb\/player\/(\d+)\//)
  const nameMatch = playerCell.match(
    /<a[^>]*href\s*=\s*["'][^"']*\/npb\/player\/\d+\/[^"']*["'][^>]*>([\s\S]*?)<\/a>/i,
  )
  const playerName = (nameMatch ? stripTags(nameMatch[1]) : stripTags(playerCell))
    .replace(/\s+/g, " ")
    .trim()
  if (!playerName) return null

  const yahooPlayerId = idMatch?.[1] ?? fallbackYahooPlayerIdForName(playerName)
...
export function parseSportsnaviStatsPlayerRows(html) {
  ...
  /** @type {{ yahooPlayerId: string | null, playerName: string, teamName?: string, yahooTeamId?: string | null, cells: string[] }[]} */
  const rows = []
  const hints = parseTeamHintsFromSportsnaviStatsHtml(html)
  const tableRe =
    /class="bb-statsTable bb-statsTable--npbTeam\d+"[\s\S]*?<tbody>([\s\S]*?)<\/tbody>/gi
  let tm
  while ((tm = tableRe.exec(html)) !== null) {
    const tableOpen = tm[0] ?? ""
    const teamNum = tableOpen.match(/bb-statsTable--npbTeam(\d+)/)?.[1] ?? ""
    const hint = teamNum ? hints.get(teamNum) : undefined
    const tbody = tm[1] ?? ""
