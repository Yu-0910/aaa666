...
function teamHistoricalDisplayName(npbLabel: string, code: string): string {
  ...
  if (npbLabel.includes("南海") || npbLabel.includes("ダイエー") || npbLabel.includes("ソフトバンク"))
    return "ソフトバンク"
  if (npbLabel.includes("西鉄") || npbLabel.includes("西武")) return "西武"
  if (npbLabel.includes("近鉄") || npbLabel.includes("オリックス")) return "オリックス"
  if (npbLabel.includes("ロッテ") || npbLabel.includes("マリーン") || npbLabel.includes("オリオン"))
    return "ロッテ"
  if (npbLabel.includes("日本ハム") || npbLabel.includes("ハム") || npbLabel.includes("ブレーブ"))
    return "日本ハム"
  if (npbLabel.includes("楽天")) return "楽天"
  if (npbLabel.includes("巨人") || npbLabel.includes("読売")) return "巨人"
  if (npbLabel.includes("阪神") || npbLabel.includes("阪急タイガ")) return "阪神"
  if (npbLabel.includes("中日") || npbLabel.includes("名古屋") || npbLabel.includes("ドラゴン"))
    return "中日"
  if (npbLabel.includes("広島") || npbLabel.includes("カープ")) return "広島"
  if (npbLabel.includes("ヤクルト") || npbLabel.includes("スワロー") || npbLabel.includes("アトム"))
    return "ヤクルト"
