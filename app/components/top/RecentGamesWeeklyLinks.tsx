    ...
    {error ? <p role="status" className="text-xs text-gray-400">週間リンクを取得できませんでした。ページを再読み込みしてください。</p> : !meta ? <p role="status" className="text-xs text-gray-400">対象週を読み込み中...</p> : <div className="flex flex-wrap gap-1.5">
      {(["CL", "PL"] as const).flatMap(league => (["batting", "pitching"] as const).map(category =>
        <Link key={`${league}-${category}`} prefetch={false}
          ...
          className="inline-flex items-center rounded border border-[#444] bg-[#141414] px-[7.2px] py-[1.8px] text-[9.9px] text-gray-400 hover:border-[#666] hover:text-[#ffff44] transition-colors focus-visible:outline focus-visible:outline-[#ffff44]">
