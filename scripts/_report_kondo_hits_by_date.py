"""近藤健介（Yahoo 1100097）日別安打集計 — 出場成績スロット（appearance_slots、ランキングと同系）"""
import json
import re
...
def appearance_slots(doc: dict, yahoo_id: str) -> list[str]:
    ...
    for line in (doc.get("domain") or {}).get("battingLines") or []:
        ...
        if isinstance(slots, list) and any(str(x or "").strip() for x in slots):
            return [str(x or "").strip() for x in slots]
...
def hits_from_slots(slots: list[str]) -> int:
    h = 0
    for raw in slots:
        t = str(raw or "").strip()
        if not t:
            continue
        if is_at_bat(t) and hit_bases(t) > 0:
            h += 1
    return h
