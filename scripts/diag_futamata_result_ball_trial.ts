/**
 * 二俣翔一: resultBallClass（打撃確定スナップ）試算 vs スポナビ公式
 *
 *   npx tsx scripts/diag_futamata_result_ball_trial.ts
 */
...
} from "../lib/yahooGame/sportsnaviFutamataSituationRef"

const root = join(fileURLToPath(new URL(".", import.meta.url)), "..")
const YAHOO = "2000066"
const REF = FUTAMATA_SITUATION_REF_2026
...
function countMismatches(bySit: Map<string, BattingSeasonAggYahoo>): number {
  ...
  for (const k of KEYS) {
    const a = bySit.get(k) ?? emptyBattingSeasonAggYahoo()
    const r = REF[k]!
    for (const field of ["ab", "h", "hr", "so", "bb", "hbp", "sh", "sf"] as const) {
      if (a[field] !== r[field]) {
    ...
    if (a.rbi !== r.rbi) {
      console.error(`  mismatch ${LABEL[k] ?? k}.rbi: got=${a.rbi} want=${r.rbi}`)
      n++
    }
  }
  ...
  const refRispAb = rispAbFromFutamataSituationRef(REF)
