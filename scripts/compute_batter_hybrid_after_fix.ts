/**
 * 打者状況別ハイブリッド試算（平川・佐藤共通）
 * npx tsx scripts/compute_batter_hybrid_after_fix.ts [yahooId]
 */
...
const REF_BY_ID: Record<string, Record<string, RefRow>> = {
  "2110164": {
  ...
  "2000051": {
    none: { pa: 123, ab: 113, h: 41, so: 0, bb: 10, hbp: 0, sh: 0, sf: 0, rbi: 9 },
    r1: { pa: 48, ab: 43, h: 14, so: 0, bb: 5, hbp: 0, sh: 0, sf: 0, rbi: 8 },
    r2: { pa: 20, ab: 12, h: 6, so: 0, bb: 8, hbp: 0, sh: 0, sf: 0, rbi: 6 },
    r3: { pa: 8, ab: 7, h: 3, so: 0, bb: 1, hbp: 0, sh: 0, sf: 0, rbi: 5 },
    r12: { pa: 14, ab: 11, h: 6, so: 0, bb: 3, hbp: 0, sh: 0, sf: 0, rbi: 6 },
    r13: { pa: 4, ab: 3, h: 0, so: 0, bb: 0, hbp: 0, sh: 0, sf: 0, rbi: 1 },
    r23: { pa: 3, ab: 3, h: 1, so: 0, bb: 0, hbp: 0, sh: 0, sf: 0, rbi: 1 },
    loaded: { pa: 5, ab: 4, h: 2, so: 0, bb: 0, hbp: 0, sh: 0, sf: 0, rbi: 5 },
  },
}
...
function main(): void {
  const yahooId = (process.argv[2] ?? "2110164").trim()
  const REF = REF_BY_ID[yahooId]
  if (!REF) {
    console.error(`Unknown yahooId: ${yahooId}`)
    process.exit(1)
  }
  ...
  let l1 = 0
  console.log(`${NAME[yahooId] ?? yahooId} — ハイブリッド試算\n`)
