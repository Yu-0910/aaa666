/**
 * 併殺打（GIDP）補完（実況>PA）影響選手について、
 * 「差分(delta)」「補完後の派生JSON上の gidp/rc」を一覧化する。
 *
 * 実行:
 *   npx tsx scripts/diag_gidp_after_fix_list.ts --year 2026
 */
...
function readDerivedGidpRc(year: string, bid: string): { gidp?: number; rc?: string } {
  const p = join(projectRoot, "_data", "derived", "player_season_batting", year, `yahoo_${bid}.json`)
  if (!existsSync(p)) return {}
  try {
    const raw = JSON.parse(readFileSync(p, "utf8")) as {
      rows?: Array<{ gidp?: number; rc?: string }>
    }
    const row = raw?.rows?.[0]
    return { gidp: row?.gidp, rc: row?.rc }
  } catch {
    return {}
  }
}
...
function main(): void {
  ...
  const rows = [...deltaByBatter.entries()]
    .map(([bid, delta]) => {
      ...
      const derived = readDerivedGidpRc(year, bid)
      return {
        bid,
        delta,
  ...
  console.log(`[gidp_after_fix] year=${year} count=${rows.length}`)
  for (const r of rows) {
    console.log(
      `- yahooId=${r.bid} +${r.delta} GIDP=${r.gidp ?? "?"} RC=${r.rc ?? "?"} ${r.name || ""} ${r.team || ""}`.trim(),
    )
  }
}
