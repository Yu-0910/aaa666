/**
 * 菊池涼介: resultBallClass（打撃確定スナップ）試算 vs スポナビ公式
 *
 *   npx tsx scripts/diag_kikuchi_result_ball_trial.ts
 */
...
} from "../lib/yahooGame/sportsnaviKikuchiSituationRef"

const root = join(fileURLToPath(new URL(".", import.meta.url)), "..")
const YAHOO = "1100082"
const REF = KIKUCHI_SITUATION_REF_2026
...
function countMismatches(bySit: Map<string, BattingSeasonAggYahoo>): number {
  ...
  const refRispAb = rispAbFromKikuchiSituationRef(REF)
...
function main(): void {
  console.log("広島・菊池涼介 (yahoo_1100082)")
